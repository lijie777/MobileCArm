...
{  
    ...
    // 一级标题
    QTreeWidgetItem *top = new QTreeWidgetItem[titleList.size()]();
    for (auto i = 0; i < titleList.size(); ++i)
    {
        top[i].setText(0, QString(titleList.at(i)));
        top[i].setCheckState(0, Qt::Checked);
        ui->treeWidget->addTopLevelItem(&top[i]);

        QTreeWidgetItem *item = new QTreeWidgetItem;
        top[i].addChild(titleInfoMap[titleList.at(i)]);
    }


    QTreeWidgetItem *titleInfo = new QTreeWidgetItem[titleInfoMap.size()]();


    // 二级标题
    for (QMap<QString, QVector<QString>>::iterator iter = titleInfoMap.begin(); iter != titleInfoMap.end(); ++iter)
    {
        top->addChild(item);
        item->setCheckState(0, Qt::Checked);
        item->setText(0, "nishishui");
    }
