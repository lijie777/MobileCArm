 ...
 * 该类主要对窗口对象进行鼠标操作事件处理
 */
class DirTxOperationTool : public QObject, public IOperationTool
{
    Q_OBJECT
...
public:
     ...
     */
    DirTxOperationTool(std::shared_ptr<DirectTextTool> drawTool, BaseView* viewer, QAbstractButton* btn = nullptr, IOperationToolPtr nxtOP = nullptr);
...
private:
    ...
    /// 文本绘制工具对象
    std::shared_ptr<DirectTextTool> m_pDrawTool;

    IOperationToolPtr m_nxtOP;  //!< 下一操作工具
};
...
* @brief 文本绘制工具回调处理类
*/
class DirTextDataCallback : public IDrawCallbackContext
{
public:
    ...
    */
    void setOperationTool(DirTxOperationTool* tool);
...
protected:
    ...
    /// 文本绘制操作工具对象
    DirTxOperationTool* m_pOperationTool;
};